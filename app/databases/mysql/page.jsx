...
export default function Home() {
  ...
  const updateDbOperations = async (operation) => {
    if (!operation) {
      return;
    }
          ...
          <div className={"custom-card-3" + (isLoading ? ' loading' : '')}>
            ...
            <div className="row">
              <div className="col d-flex">
                <button
                  onClick={() => {
                    const action = databaseDataLocal.active ? 'stop' : 'start';
                    updateDbOperations(action);
                  }}
                  className="btn btn-light mr-1"
                  disabled={!databaseDataLocal.installed}
                >
                  {
                    databaseDataLocal.active
                      ? <MdSquare className="action-icon" />
                      : <MdPlayArrow className="action-icon" />
                  }
                </button>
                <button
                  onClick={() => updateDbOperations('restart')}
                  className="btn btn-light mr-1"
                  disabled={databaseDataLocal.installed}>
                  {
                    databaseDataLocal.active
              ...
              <div className="col ">
                <Select
                  ...
                  options={
                    dbUsers
                      .filter((user) => {
                        const isMs = pageServiceName === 'mysql' ? true : false;
                        const dbObj = databases.find((db) => db.name === newUserToDbData.dbName);
                        if (isMs && dbObj && dbObj.user === user.name) {
                          return false;
                        } else {
                          return true;
                        }
